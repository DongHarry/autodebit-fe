<template>
  <div class="container-layout">
    <div class="container-layout__header">
      <Breadcrumb :paths="title.BREADCRUMB_EDIT_VENDOR.value">
    ...
    <div class="container-layout__content">
      <a-tabs>
        <a-tab-pane key="1" :tab="$t('vendor.general_info')">
          <a-form
            ref="formVendor"
            layout="vertical"
            :model="formState"
        ...
        <a-tab-pane key="2" :tab="$t('vendor.vendor_contract')"> </a-tab-pane>
      </a-tabs>
    </div>
  </div>
</template>
...
import { updateVendor, showVendor } from '~/api/vendors'
...
const { submit, serverErrors, fieldChange, loading } = useFormSubmit({
  formRef: formVendor,
  apiSubmit: updateVendor,
  formState,
})

onMounted(async () => {
  const route = useRoute()
  const response = await showVendor(route.params.id as string)
